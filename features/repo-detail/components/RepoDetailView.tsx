    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <StatCard icon={<Star className="w-6 h-6 text-yellow-500" />} label="Stars" value={repo.stargazers_count} />
          <StatCard icon={<Eye className="w-6 h-6 text-blue-500" />} label="Watchers" value={repo.subscribers_count} />
          <StatCard icon={<GitFork className="w-6 h-6 text-purple-500" />} label="Forks" value={repo.forks_count} />
          <StatCard icon={<CircleDot className="w-6 h-6 text-green-500" />} label="Issues" value={repo.open_issues_count} />
        </div>
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {repo.topics.map((topic) => (
                <span key={topic} className="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-sm font-medium hover:bg-gray-200 transition-colors">
                  {topic}
                </span>
              ))}
            </div>
