...
- **Server Component & Client Component**:
  - インタラクティブな検索フォームやリストコンポーネントはClient Componentとして実装
  - 検索フォームはシンプルなUncontrolled Inputで実装し、FormDataで値を取得することでバンドルサイズを最小化
  - バリデーション（100文字制限）は軽量なステート管理で実現し、React Hook Formなどの重いライブラリを避けています（YAGNI原則）
  - 検索結果の状態管理にuseSWRを使用。検索一覧画面 >> 検索詳細画面 >> 戻るボタン押下時に、keepPreviousDataの設定で検索結果のキャッシュを実現
  - **useEffectを完全排除**: URLをSingle Source of Truthとして扱い、useMemo/useCallbackでderived stateを実装することで、同期バグやメモリリークのリスクを根本から排除
  - 検索一覧取得は利用上限のあるAPIのため、一律20件取得するように設計（今後拡張でper_pageの長さを選択できるように設計）
...
- **URL クエリでの状態管理**:
  - URLのクエリで検索クエリを保存することで、「検索一覧画面」 >> 「検索詳細画面」 >> 「検索一覧画面」の画面遷移のときに、検索結果の状態の維持を可能にします。
- **堅牢なバリデーション (Zod)**:
...
```mermaid
flowchart TD
    A["（人）設計 (READMEの記載)"] --> B["（AI）READMEをもとにコードを生成"]
    B --> C["（人）成果物の確認"]
    C --> D["（人）課題のリスト化"]
    D --> E["（AI）記載したtodoを実装"]
    E --> F["（人/AI）レビュー、動作確認"]
    F --> G["（人/AI）テスト拡張"]
    G --> H["（人/AI）README.mdの更新"]
```
